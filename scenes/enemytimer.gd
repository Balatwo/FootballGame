extends Timer
signal icandancelikemichaeljackson
...
const SPAWN_DELAY: float = 1 # Time in seconds between spawns
...
func _on_spawn_timer_timeout():
	spawn_next_enemy()
	$".".wait_time = max($".".wait_time - 10, 0.0001)
	emit_signal("icandancelikemichaeljackson")
